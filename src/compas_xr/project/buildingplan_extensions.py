...
class BuildingPlanExtensions(object):
    ...
    def create_buildingplan_with_custom_sequence(self, assembly, sequenced_keys, data_type, robot_keys, priority_keys_lists):
        """
        Create a compas_timber.planning.BuildingPlan based on the sequence of the assembly parts.
        ...
        data_type : int
            Index of which data type will be loaded on the application side [0: 'Cylinder', 1: 'Box', 2: 'ObjFile']
        robot_keys : list of str
            List of keys that are intended to be built by the robot.
        priority_keys_lists : list of list of str
            List in assembly order of lists of assembly keys that can be built in parallel.
    ...
    def create_buildingplan_with_custom_sequence_and_mixed_data_types(self, assembly, sequenced_keys, robot_keys=None, priority_keys_lists=None):
        """
        Create a compas_timber.planning.BuildingPlan based on the sequence of the assembly parts.
        ...
        for key in sequenced_keys:
            ...
            # TODO: This is dumb, but the element_ids are generated incorrectly so they are overwritten here
            step.element_ids = [key]
            step.geometry = node_data[str(key)]["part"].__data__["attributes"]["type"]
            # TODO: These are unused for now, but are expeted on the application side
            step.instructions = node_data[str(key)]["part"].__data__["attributes"].get("ObjName", "none")
            step.elements_held = [0]
            step.location = node_data[str(key)]["part"].frame
            ...
            if "placed_by" in node_data[str(key)]["part"].__data__["attributes"]:
                step.actor = node_data[str(key)]["part"].__data__["attributes"]["placed_by"]
